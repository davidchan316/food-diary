...
import com.techelevator.foodiary.dao.JdbcUserDao;
...
public class JdbcUserDaoTests extends BaseDaoTests {
    protected static final User USER_1 = new User(1001, "user1", "user1", "First1", "Last1", "USER");
    protected static final User USER_2 = new User(1002, "user2", "user2", "First2", "Last2", "USER");
    private static final User USER_3 = new User(1003, "user3", "user3", "First3", "Last3", "USER");
    ...
    public void create_user_with_null_username() {
        sut.create(null, USER_3.getPassword(), "First", "Last");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_existing_username() {
        sut.create(USER_1.getUsername(), USER_3.getPassword(), "First", "Last");
    }

    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        sut.create(USER_3.getUsername(), null, "First", "Last");
    }

    @Test
    public void create_user_creates_a_user() {
        User newUser = new User(-1, "new", "user", "First", "Last", "USER");

        boolean userWasCreated = sut.create(newUser.getUsername(), newUser.getPassword(), newUser.getFirstName(), newUser.getLastName());
